  ...
  head: () => ({
    meta: [
      { title: "La Ruta del Mate — Yerbas seleccionadas en Suipacha" },
      {
        name: "description",
        content:
          "Emprendimiento familiar de Suipacha. Yerbas uruguayas y brasileñas seleccionadas: Baldo, Canarias, Rei Verde, Sara y más.",
      },
      { property: "og:title", content: "La Ruta del Mate — Yerbas seleccionadas" },
      {
        property: "og:description",
        content: "Yerbas uruguayas y brasileñas seleccionadas en Suipacha.",
      },
    ],
    links: [
      {
    ...
    <>
          ...
          <div className="min-w-0">
            ...
            <div className="mt-10 flex flex-wrap items-center gap-6 text-xs text-primary-foreground/70 rm-anim-fade-up rm-delay-500">
              <span className="flex items-center gap-2">
                <MapPin className="h-4 w-4 text-[color:var(--gold)]" /> Retiro en Suipacha
              </span>
              <span className="flex items-center gap-2">
                <Truck className="h-4 w-4 text-[color:var(--gold)]" /> Envíos coordinados
              </span>
              <span className="flex items-center gap-2">
                <Sparkles className="h-4 w-4 text-[color:var(--gold)]" /> Selección artesanal
              </span>
            </div>
          </div>
          <div className="relative rm-anim-fade-in rm-delay-200">
            <div
              className="absolute -inset-6 rounded-[2rem] bg-[color:var(--gold)]/20 blur-3xl rm-anim-float"
              aria-hidden
            />
        ...
        <div className="grid gap-12 md:grid-cols-[1fr_1.3fr] md:items-center">
          <div>
            <div className="relative mx-auto h-56 w-56 md:mx-0">
              <div
                className="absolute -inset-4 rounded-full bg-[color:var(--gold)]/25 blur-2xl rm-anim-float"
                aria-hidden
              />
              <img
                src={SITE.logo}
          ...
          <div>
            ...
            <p className="mt-4 text-base leading-relaxed text-muted-foreground">
              La Ruta del Mate es un proyecto familiar que arrancó con una idea simple: acercar las
              yerbas que más nos gustan a la mesa de todos los días. Traemos padrones uruguayos y
            ...
            {featured.map((p, idx) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {p.tag && (
                    <span className="rounded-full bg-[color:var(--gold)]/20 px-3 py-1 text-xs font-medium text-[color:var(--gold-foreground)]">
                      {p.tag}
                    </span>
                  )}
                </div>
                ...
                <div className="mt-5 space-y-1.5">
                  {p.variants.map((v) => (
                    <div
                      key={v.size}
                      className="flex items-baseline justify-between border-b border-dashed border-border/70 py-1 text-sm"
                    >
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-3xl bg-primary p-8 text-primary-foreground shadow-warm sm:p-12 md:p-16">
          <div
            className="absolute -right-16 -top-16 h-64 w-64 rounded-full bg-[color:var(--gold)]/25 blur-3xl"
            aria-hidden
          />
          <div className="relative grid gap-8 md:grid-cols-[1.4fr_1fr] md:items-center">
            <div>
              ...
              <p className="mt-3 max-w-xl text-primary-foreground/80">
                Escribinos por WhatsApp y te asesoramos según tu perfil de mate — suave, intenso,
                con hierbas o algo distinto.
              </p>
            </div>
            <div className="flex flex-wrap gap-3 md:justify-end">
              ...
              <Link
                to="/catalogo"
                className="inline-flex items-center gap-2 rounded-full border border-white/30 bg-white/5 px-6 py-3 text-sm font-semibold text-white hover:bg-white/10"
              >
                Ver catálogo
              </Link>
            </div>
